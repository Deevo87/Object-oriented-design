...
public class OrderTest {
	...
	public void testGetProductThroughOrder() {
		...
		// when
		Product actualProduct = order.getProduct(expectedProduct.getName());
	...
	public void testGetPrice() throws Exception {
		...
		// when
		BigDecimal actualProductPrice = order.getPrice(product.getName());
	...
	public void testPriceWithTaxesWithoutRoundUp() {
		...
		// then
		for (Map.Entry<String, Product> p : order.getProducts().entrySet()) {
			String tempProd = p.getValue().getName();
			assertBigDecimalCompareValue(order.getPriceWithTaxes(tempProd), BigDecimal.valueOf(2.46)); // 2.44 PLN
		}
	}
	...
	public void testPriceWithTaxesWithRoundDown() {
		...
		for (Map.Entry<String, Product> p : order.getProducts().entrySet()) {
			String tempProd = p.getValue().getName();
			assertBigDecimalCompareValue(order.getPriceWithTaxes(tempProd), BigDecimal.valueOf(0.01)); // 0.01 PLN
		}
	}
	...
	public void testPriceWithTaxesWithRoundUp() {
		...
		for (Map.Entry<String, Product> p : order.getProducts().entrySet()) {
			String tempProd = p.getValue().getName();
			assertBigDecimalCompareValue(order.getPriceWithTaxes(tempProd), BigDecimal.valueOf(0.04)); // 0.04 PLN
		}
